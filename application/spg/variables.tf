...
# To revert to the local.ansible_vars["spg_jms_host"] variable, add spg_jms_host_src = "var" to the delius
# env-configs for an environment where there is no SPG AmazonMQ broker
variable spg_jms_host_src {
  default     = "data"
  description = "var -> local.ansible_vars[spg_jms_host] | data -> data.terraform.remote_state.amazonmq.amazon_mq_broker_connect_url"
}