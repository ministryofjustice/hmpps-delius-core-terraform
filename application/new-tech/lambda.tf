locals {
  lambda_name         = "${var.environment_name}-${local.app_name}-nightly-restart"
  schedule_expression = "cron(15 2 * * ? *)" # 02:15 every day
}
...
resource "aws_lambda_function" "nightly_restart_lambda" {
  ...
  environment {
    variables = {
      ...
      DESIRED_COUNT = lookup(local.app_config, "min_capacity", var.common_ecs_scaling_config["min_capacity"])
    }
  }
}