module "external_nlb" {
  ...
  eip_allocation_ids = [
    data.terraform_remote_state.persistent-eip.outputs.delius_interface_az1_lb_eip.allocation_id,
    data.terraform_remote_state.persistent-eip.outputs.delius_interface_az2_lb_eip.allocation_id,
    data.terraform_remote_state.persistent-eip.outputs.delius_interface_az3_lb_eip.allocation_id,
  ]

  haproxy_security_groups = [
    data.terraform_remote_state.vpc_security_groups.outputs.sg_ssh_bastion_in_id,
    data.terraform_remote_state.delius_core_security_groups.outputs.sg_weblogic_interface_lb_id,
