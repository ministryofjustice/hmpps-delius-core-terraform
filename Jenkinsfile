...
boolean confirm(String component) {
	...
	def changes = sh(script: "grep 'Plan:' '${component}/.terraform/output/${env.ENVIRONMENT}/tg.log' | col -b | sed -E 's/[0-9]+m//g'", returnStdout: true).trim()
	try {
...
pipeline {
	...
	parameters {
		...
		booleanParam(name: 'deploy_DATABASE_HA', description: 'Deploy/update Database High Availability?', defaultValue: true)
	...
	environment {
		CONFIG_LOCATION = "${env.WORKSPACE}/${project.config}"
	...
	stages {
		...
		stage ('Core Services') {
			parallel {
				stage('WebLogic - ndelius') { steps { do_terraform(project.source, 'application/ndelius') } }
				stage('WebLogic - spg') { steps { do_terraform(project.source, 'application/spg') } }
				stage('WebLogic - interface') { steps { do_terraform(project.source, 'application/interface') } }
				stage('Batch - DSS') { steps { do_terraform(project.source, 'batch/dss') } }
			}
		}
