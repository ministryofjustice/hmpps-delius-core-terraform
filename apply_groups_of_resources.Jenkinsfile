...
def confirm() {
    try {
        timeout(time: 15, unit: 'MINUTES') {
            slackSend(message: "\"Apply\" of \"${resource_name}\" on \"${environment_name}\" - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL.replace(':8080','')}console|Paused for input>)")
...
pipeline {
    ...
    parameters {
        ...
        choice(
          name: 'resource_name',
          choices: resources,
          description: 'Select resource for applying.'
        )

        string(
          name: 'hmpps_env_configs_branch',
          defaultValue: "master",
          description: 'The branch name of the hmpps-env-configs project.'
        )

        string(
          name: 'hmpps_delius_core_terraform_branch',
          defaultValue: "master",
          description: 'The branch name of the hmpps-env-configs project.'
        )
    }
    ...
    stages {
        ...
        stage('setup') {
            steps {
              slackSend(message: "\"Apply\" of \"${resource_name}\" started on \"${environment_name}\" - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL.replace(':8080','')}|Open>)")

              dir( project.config ) {
                git url: 'git@github.com:ministryofjustice/' + project.config, branch: "${hmpps_env_configs_branch}", credentialsId: 'f44bc5f1-30bd-4ab9-ad61-cc32caf1562a'
              }
              dir( project.dcore ) {
                git url: 'git@github.com:ministryofjustice/' + project.dcore, branch: "${hmpps_delius_core_terraform_branch}", credentialsId: 'f44bc5f1-30bd-4ab9-ad61-cc32caf1562a'
              }
                prepare_env()
            }
        }
        ...
        stage('Plan & Apply change') {
          steps {
            script {
              do_terraform(project.config, environment_name, project.dcore, resource_name)
            }
          }
        }
    ...
    post {
        ...
        success {
            slackSend(message: "\"Apply\" of \"${resource_name}\" completed on \"${environment_name}\" - ${env.JOB_NAME} ${env.BUILD_NUMBER} ", color: 'good')
        }
        failure {
            slackSend(message: "\"Apply\" of \"${resource_name}\" failed on \"${environment_name}\" - ${env.JOB_NAME} ${env.BUILD_NUMBER} ", color: 'danger')
        }
    }
