...
resource "aws_instance" "loadrunner" {
  ...
  tags = merge(
    var.tags,
    {
      "Name" = "${data.terraform_remote_state.vpc.outputs.environment_name}-loadrunner"
    },
  )
  ...
  vpc_security_group_ids = [
    ...
    data.terraform_remote_state.delius_core_security_groups.outputs.sg_loadrunner_id,
  ]
  root_block_device {
