locals {
  alarm_name = "${var.environment_name}-StatusCheckFailed-${module.delius_db_3.ami_id}"
}

resource "aws_cloudwatch_log_group" "ec2_status_check_log_group" {
  name              = "/metrics/${var.environment_name}/ec2-db-status-check-failed-${module.delius_db_3.ami_id}"
...
resource "aws_cloudwatch_metric_alarm" "status_check_failed_alarm" {
  ...
  dimensions = {
    InstanceId = module.delius_db_3.ami_id
  }
}
